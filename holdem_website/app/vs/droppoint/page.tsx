...
const faqs = [
  {
    ...
    a: 'Not actively, as far as its public repo shows: the latest release is v1.2.1 (September 2022) with no newer release since, and the last code push was in 2023. It still works, but bugs and OS changes since then are unaddressed. Holdem is under active development (v3.0.0).',
  },
  {
    ...
    <main className="mx-auto max-w-3xl py-8">
      ...
      <article className="prose prose-zinc mt-6 max-w-none dark:prose-invert prose-headings:tracking-tight prose-a:text-zinc-900 dark:prose-a:text-zinc-100">
        ...
        <p className="lead text-zinc-600 dark:text-zinc-400">
          ...
          GitHub stars. But its last release was v1.2.1 in September 2022, it runs
          on heavyweight Electron, and summoning it means an awkward
        ...
        <div className="not-prose rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          Linux right now? DropPoint still does the job. On Windows alone, Holdem
          wins on weight (Tauri/Rust vs Electron), trigger (shake vs
          Shift+CapsLock), maintenance (v3.0.0 vs a 2022 release), and extras like
          browser-image drops.
        </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              ...
              <tr>
                ...
                <td className="px-4 py-3">Tauri + Rust + TypeScript — tiny installer, near-zero background CPU</td>
                <td className="px-4 py-3">Electron + Node.js — release installers run ~54–78MB per asset</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-medium">Trigger</td>
                <td className="px-4 py-3">Mouse shake mid-drag or global hotkey; shelf appears at cursor</td>
                <td className="px-4 py-3">Shift+CapsLock anywhere (Shift+Tab on macOS) or tray icon; no shake gesture</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td className="px-4 py-3">Quiet tray app, auto-launch, one floating shelf that auto-hides</td>
                <td className="px-4 py-3">Minimizes to tray; supports multiple instances; new instances open at mouse location</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-medium">Virtual desktops</td>
                <td className="px-4 py-3">Tray-based, reachable everywhere</td>
                <td className="px-4 py-3">Windows needs a manual Task-view step (“Show this window on all desktops”)</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-medium">Maintenance</td>
                <td className="px-4 py-3">Active — current release v3.0.0</td>
                <td className="px-4 py-3">Stalled — last release v1.2.1 (Sep 2022), last push 2023</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-medium">Design</td>
                <td className="px-4 py-3">Minimal dark floating shelf</td>
                <td className="px-4 py-3">Functional multi-window Electron UI (taste differs — try both)</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <p>
          ...
          Slack and VS Code — and its own release page tells the story: a Windows
          zip around 77MB, a macOS dmg up to ~79MB. That is fine for a main app,
          but heavy for a utility whose job is to hold three files for thirty
          seconds. Holdem is built with Tauri and Rust, which reuses the system
          webview instead of shipping Chromium: the result is a far smaller
          install, negligible idle CPU, and instant summon. Both approaches work;
          only one disappears into the tray.
        </p>

        <h2>Trigger: shake vs Shift+CapsLock</h2>
        <p>
          This is the daily-experience gap. With DropPoint you press{' '}
          <code>Shift + Caps Lock</code> (an unusual chord that collides with typing
          habits) or dig out the tray icon — and on Windows, if you use virtual
          desktops, you must first perform a one-time Task-view ritual to pin the
          window across desktops. With Holdem you simply shake the mouse while
          already dragging — the shelf materializes exactly where your cursor is —
          or tap the hotkey. No chord to memorize, no per-desktop setup, nothing
          between intention and shelf.
        </p>
        ...
        <p>
          ...
          activity in 2023, 28 open issues — means Windows 11 changes, new browsers,
          and HiDPI quirks land in an app nobody is patching. That is not an attack:
          open-source maintainers move on, and the GPL code remains available for
          anyone to fork. But if you are choosing a shelf to rely on daily, an
          actively developed app (Holdem v3.0.0, Tauri auto-updates via{' '}
          <code>update.json</code>) is the safer bet — which is exactly why Holdem
          exists rather than as a DropPoint fork.
        </p>

        <h2>Where DropPoint honestly wins</h2>
        <p>
          Cross-platform. Full stop. If your workflow spans Windows, macOS, and
          Linux, DropPoint is the only free open-source shelf covering all three,
          and its multiple-instances model suits people who keep several staging
          piles open. The GPL-3.0 license also guarantees the code stays open,
          which matters to free-software purists (Holdem&apos;s MIT license is more
          permissive instead). Windows-only users give up nothing by switching.
        </p>
        ...
        <h2>Sources</h2>
        <ul>
          <li><a href="https://github.com/GameGodS3/DropPoint" target="_blank" rel="noopener noreferrer">DropPoint on GitHub</a> — GPL-3.0, Electron, Shift+CapsLock / Shift+Tab triggers, tray behavior, virtual-desktop step</li>
          <li><a href="https://github.com/GameGodS3/DropPoint/releases/tag/v1.2.0" target="_blank" rel="noopener noreferrer">DropPoint v1.2.0 release assets</a> — installer sizes (~54–79MB across platforms)</li>
          <li><a href="https://droppoint.netlify.app/" target="_blank" rel="noopener noreferrer">DropPoint homepage</a> — feature rundown, hotkey, tray</li>
        </ul>
